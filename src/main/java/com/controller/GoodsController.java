...
public class GoodsController {
	...
	public String goodsList (HttpServletRequest request) {
		...
		goodsService.queryAll(request);
		typeService.queryAllForGOODS(request);						//****获取分类列表，用大写的GOODS，是为了在查找替换时不被更改
	...
	public String goodsShow (HttpServletRequest request) {
		...
		goodsService.queryByGoodsId(request);						//****根据ID获取记录
		...
		image = goodsImageService.getImageTag(request, image);		//****生成商品图片标签和图片链接 
	...
	public String goodsShowImageRenew (HttpServletRequest request) {
		...
		goodsService.queryByGoodsId(request);
	...
	public String goodsAdd (HttpServletRequest request) {
		...
		typeService.queryAllForGOODS(request);						//****获取分类列表
	...
	@RequestMapping(value="/goodsAddDo", produces={"text/html;charset=UTF-8"})		//避免返回的中文内容为乱码
	...
	public String goodsAddDo (@Validated Goods goods, Errors errors, HttpServletRequest request) {
		...
		String goodsId = goodsService.addGoods(goods, request);
		
		if (goodsId.equals("0")) {									//新添失败
			msg = (String) request.getAttribute("msg");
			return msg;												//输出失败消息
		}
		
		return "@Redirect:goodsShow?goodsId=" + goodsId;			//新添成功，将由JS实现网页跳转
	}
	...
	public String goodsEdit (HttpServletRequest request) {
		...
		goodsService.queryByGoodsId(request);						//****获取该记录
		typeService.queryAllForGOODS(request);						//****获取分类列表
	...
	public String goodsEditDo (@Validated Goods goods, Errors errors, HttpServletRequest request) {
		...
		boolean result = goodsService.editGoods(goods, request);	//****更新记录
		...
		return "@Redirect:goodsShow?goodsId=" + goods.getGoodsId();	//更新成功
	}
	...
	public String goodsDeleteDo (HttpServletRequest request) {
		...
		goodsService.deleteByGoodsId(request);
		
		return "redirect:goodsList";								//重定向到商品列表页
	}
}
