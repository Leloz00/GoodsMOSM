...
public class GoodsServiceImpl implements GoodsService {
	...
	public boolean selectAll (HttpServletRequest request) {
		...
		try {
			if (request.getParameter("buttonDelete") != null) { 				//如果单击了删除按钮
				...
				String[] goodsIdArray = null; 	//####【修改代码】	//获取ID列表	
			...
			int countRow = goodsDao.selectCount(search);	//****获得记录总数
			...
			goodsList = goodsDao.selectAll(search, countShowed, pageSize);	//****获取当前页的记录列表
	...
	public boolean selectByGoodsId (HttpServletRequest request) {
		...
		try {
			String goodsId = "";	//####【修改代码】 		//获取地址栏参数goodsId的值 
			
			try { 									 
				//####【添加代码】 							//尝试转换为整数，即判断其是否为整数
			} catch (Exception e) {
				msg = "参数goodsId错误！";
				return false;
			}	
			
			goods = null;	//####【添加代码】			//****根据ID获取记录
	...
	public boolean existByGoodsNo (Goods goods, HttpServletRequest request) {
		...
		try {
			...
			Goods goodsSelect = goodsDao.selectByGoodsNo(goodsNo);				//****根据goodsNo获取记录
	...
	public boolean existByGoodsNoExceptGoodsId (Goods goods, HttpServletRequest request) {
		...
		try {
			...
			Goods goodsSelect = goodsDao.selectByGoodsNoExceptGoodsId(goodsNo, goodsId);	//****获取记录
	...
	public String insertGoods (Goods goods, HttpServletRequest request) {
		...
		try {
			...
			if (imageUUID != null && imageUUID.equals("") == false) {
				...
				int count = 0;		//####【修改代码】		//****更新记录。目的是根据goodsId更新image字段
	...
	public boolean updateGoods (Goods goods, HttpServletRequest request) {
		...
		try {	
			...
			//-------------------------------------- 根据typeId获取typeName
			Type type = typeDao.selectByTypeId(typeId);					//****
	...
	public boolean deleteByGoodsId (HttpServletRequest request) {
		...
		try {	
			...
			if (true) {						//####修改代码						//如果已存在同名文件，应先删除它
				if (false == false) {		//####修改代码						//如果删除失败
					msg += "将文件" + image + "删除时失败。";
					return false;
				}
			}
	...
	public boolean deleteByGoodsIdLot (String[] goodsIdArray, HttpServletRequest request) {		//被本类中的selectAll()方法调用
